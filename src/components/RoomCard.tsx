...
export default function RoomCard({ room }: RoomCardProps) {
  ...
  const imageHint = room.photos && room.photos.length > 0 ? room.title.substring(0,20) : "room interior";
  ...
  return (
    <Card className="flex flex-col h-full overflow-hidden rounded-lg shadow-lg hover:shadow-xl transition-shadow duration-300">
      ...
      <CardContent className="p-4 flex-grow">
        <Link href={`/room/${room.id}`} className="block">
          <CardTitle className="text-lg font-semibold mb-2 hover:text-primary transition-colors line-clamp-2">{room.title}</CardTitle>
        </Link>
        <div className="text-sm text-muted-foreground space-y-1">
          <div className="flex items-center">
            <MapPin className="h-4 w-4 mr-2 text-accent" />
            <span>{room.address_1}, {room.city}</span>
          </div>
          <div className="flex items-center">
            <BedDouble className="h-4 w-4 mr-2 text-accent" />
            <span>{room.room_type_name} en {room.property_type_name}</span>
          </div>
